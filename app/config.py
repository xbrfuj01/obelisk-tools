...
# The only thing that stays as an env var: it's a Docker volume mount
# point, not an application setting, so it belongs to the deployment, not
# the admin panel. Core has no /downloads volume of its own anymore -
# downloader-converter and metadata each keep their own file storage in
# their own container/volume now.
...
# reachable from inside this container the same way bgutil-provider is.
# A module that isn't deployed simply fails to resolve/connect - see
# modules.py, which turns that into "hide this tool" rather than an error.
